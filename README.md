...
一个 Windows 桌面悬浮小窗：启动 Claude Code / dsh 时自动弹出，实时显示 DeepSeek 开放平台的**官方用量**（余额、今日消费、模型 Tokens、请求数）和本地缓存命中统计。

数据以**官网为准**（Playwright 抓 `platform.deepseek.com/usage`），本地日志只做模型分解与 7 天图表，避免"本地统计 ≠ 官方"的偏差。
...
DeepSeek V4 系列在 **2026-08-17 起大幅涨价**并改为峰谷定价——高峰时段费用直接翻倍，跑模型的成本一下子敏感起来（网友戏称"梁圣变梁子"，说的就是这次调价）。

涨价之后，"得盯着点花费"就成了日常。但每次想看余额和用量，都得打开浏览器登录官方平台翻页面——重复几次就烦了。于是做了这个小弹窗：**启动 Claude Code / dsh 时自动弹出，官方余额、今日消费、模型 Tokens 一眼可见**，省掉反复登录官网的麻烦，随时知道钱花在哪儿。

## 功能

- 🪟 无边框置顶小窗，贴屏幕右上角，可拖拽
- 💰 官方余额 / 当日消耗 / 今日 Tokens
- ⚡ V4 Flash / V4 Pro 分模型卡片（官方 Tokens + 本地缓存命中率）
- 📊 最近 7 天缓存命中明细柱状图
...
官网抓取需要 DeepSeek 开放平台登录会话，首次执行弹窗登录一次：

```bash
node fetch_official.js --login   # 在弹出的 Edge 窗口登录，会话存 .edge-profile/
node fetch_official.js           # 之后静默抓取，输出官方今日 JSON
```
...
- **官方为准**：余额 / 今日消费 / Tokens / 请求数 / per-model 来自 `platform.deepseek.com/usage`（抓"今天"+"模型"视图）。官网 API 只有 `/user/balance`，没有用量端点，必须爬网页。
- **本地分解**：`data/parse_usage.py` 聚合 Claude Code 会话 jsonl，提供模型卡片的缓存命中率与 7 天图表。**不**再把缓存命中重读混进"用量"（缓存复用 98% 会把数字撑爆）。
- **费用折算**：`data/prices.py` 按消息时间戳分段计价。DeepSeek V4 2026-08-17 起改**峰谷定价**（高峰=北京 9-12、14-18 点），`prices.py` 已内置新旧两套价目并自动分段。
- 若官网抓不到（未登录/超时），余额回退 DeepSeek API，其余字段如实显示 "--"，不编造。
...
├── fetch_official.js    # 官网用量抓取器（Playwright + Edge profile）
├── config.example.json  # 配置模板（复制为 config.json 后填写）
├── data/
│   ├── config.py        # 读取 config.json / 默认值
│   ├── prices.py        # V4 峰谷价目 + 分段计价
...
- 🍪 `.edge-profile/`（登录会话）与真实账户截图**已被 .gitignore 排除**，不会入库
